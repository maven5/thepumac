	...
	var v_idx = $('#v_idx').val(); // v_idx 
	 ...
	 $(document).on("click",".itemSave",function(){  // 아이템 저장버튼
		...
		if(v2_idx!=0) { // update( 기존 항목 수정 )
			 if(confirm('기존에 있던 항목입니다. \n수정하시겠습니까?'))
				 location.href="/action/vote/vote_small_update.php?v_idx="+v_idx+"&v2_idx="+v2_idx+"&v2_subject="+v2_subject;
		}else{ // insert ( 새 항목 추가 )
			 if(confirm('새 항목을 추가하시겠습니까?'))
				 location.href="/action/vote/vote_small_insert.php?v_idx="+v_idx+"&v2_idx="+v2_idx+"&v2_subject="+v2_subject;
		}
	 });
	 ...
	 $(document).on("click",".itemDelete",function(){  // 아이템 삭제버튼
		 ...
		 if(item==1){
			 ...
			 if(confirm('기존에 있던 항목입니다. \n삭제하시겠습니까?'))
				 location.href="/action/vote/vote_small_delete.php?v_idx="+v_idx+"&v2_idx="+v2_idx;
			 else
			 	return false;
		 }
